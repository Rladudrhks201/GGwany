...
image_path = 'C:\\Users\\user\\Desktop\\project\\fruits1\\'
...
def cutmix(path, index, imsize):
    ...
    # 검은색 배경의 임의 이미지 생성 (여기다가 이미지 붙여넣기)
    ...
    for i, index in enumerate(indexes):
        ...
        if i == 0:
            # large image
            x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - w, 0), xc, yc
            # small image
            x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h
        elif i == 1:
            # top right
            x1a, y1a, x2a, y2a = xc, max(yc - w, 0), min(xc + w, s * 2), yc
